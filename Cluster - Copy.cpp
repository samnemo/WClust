...
{	
	...
	//one kdtree for all clusters
	KDTreeHist oTree;
	{ ScopedTimer S("build full tree");
	...
	/*for(k=0;k<vBinData.size();k++)
	{
		int l = 0;
		for(l=4;l<=7;l++)   vBinData[k][l] = 0;
		for(l=12;l<=15;l++) vBinData[k][l] = 0;
	}*/
	...
	for(iRun = 0; iRun < iAlgorithmRuns && !pCUPDUPData->ShouldTerminate(); iRun++)
	{
		...
		vector<int> vClustIDs(DataStack.m_VerxStack.size()), vCounts(iClusts+1);
		...
		while(true)
		{
			...
			for(iV=0;iV<vClustIDs.size();iV++,iJnk++)
			{
				...
				vEntropyTempA[iOrig]=((vCounts[iOrig]-1.0)/(prob_t)iRows)*oTree.Entropy(vClustIDs,iOrig,vCounts[iOrig]-1.0);
				...
				for(iT=1;iT<=iClusts;iT++)
				{
					...
					vEntropyTempB[iT]=((vCounts[iT]+1.0)/(prob_t)iRows)*oTree.Entropy(vClustIDs,iT,vCounts[iT]);
				...
				if(iMinInd != iOrig)
				{
					...
					vClustIDs[iV]=iMinInd;
					vCounts[iMinInd]++;
					vcEntropy[iMinInd]=((prob_t)vCounts[iMinInd]/(prob_t)iRows)*oTree.Entropy(vClustIDs,iMinInd,vCounts[iMinInd]);

					//move vertex OUT of original distribution
					vCounts[iOrig]--;
					vcEntropy[iOrig]=((prob_t)vCounts[iOrig]/(prob_t)iRows)*oTree.Entropy(vClustIDs,iOrig,vCounts[iOrig]);
