...
void CCluster::CalcClusterInfo(CVerxStack& DataStack,bool bAllBins)
{
	DataStack.CalcMinMax();

	int iClusts = GetNumClusts(DataStack) , iC=1;
	...
	if(bAllBins)
	{
		int iNumBins = 10, iBinInc = 20 , iMaxBins = 210, iNumIters = 11;
		int iInfPerClust = iNumIters * (2 + m_iInfPerClust), iOff = 0;
		
		m_vInfo[DataStack.whichDraw] = vector< vector<double> >(iClusts+1);
		for(iC=1;iC<=iClusts;iC++) m_vInfo[DataStack.whichDraw][iC] = vector<double>(iInfPerClust);

		for(iNumBins=10;iNumBins<=iMaxBins;iNumBins+=iBinInc,iOff+=(2+m_iInfPerClust))
		{
			//fill distributions once for each number of bins
			vector< vector<Hist> > vDistribs;
			FillDistribs(DataStack,*this,iNumBins,vDistribs,iClusts+1,DataStack.whichDraw);
			...
			{
				double kldiv=0.0;
				for(iD=0;iD<iDims;iD++) kldiv += KLDiv(vDistribs[iC][iD],vDistribs[iClusts+1][iD]);			
				//non-weighted inclusive info gain
				m_vInfo[DataStack.whichDraw][iC][iOff+0] = kldiv;
				//weighted inclusive info gain
				m_vInfo[DataStack.whichDraw][iC][iOff+1] = ( (double) vCounts[iC] / iNNVertices ) * kldiv;
			}
			//uniqueness+distinction distance measure
			vcInf=vector<double>(iClusts+1);
			vector< vector<double> > vcInfInter;
			CalcUDDist(vDistribs,iClusts,vcInf,vcInfInter,vCounts,iNNVertices,vZeroes,false);
			for(iC=1;iC<=iClusts;iC++)
			{
				m_vInfo[DataStack.whichDraw][iC][iOff+2] = vcInf[iC];
				m_vInfo[DataStack.whichDraw][iC][iOff+3] = ( (double) vCounts[iC] / iNNVertices ) * vcInf[iC];
			}
			//entropy
			vector<double> vcEntropy(iClusts+1);
			CalcEntropy(vDistribs,vcEntropy,iClusts,iDims,vCounts,iNNVertices,vZeroes,false);
			for(iC=1;iC<=iClusts;iC++)
			{	//un-weighted entropy
				m_vInfo[DataStack.whichDraw][iC][iOff+4] = vcEntropy[iC];
				//weighted entropy
				m_vInfo[DataStack.whichDraw][iC][iOff+5] = ( (double) vCounts[iC] / iNNVertices ) * vcEntropy[iC];
			}
			...
			{
				double kldiv=0.0;
				for(iD=0;iD<iDims;iD++) kldiv += KLDivApproxExclusiveProb(vDistribs[iC][iD],vDistribs[iClusts+1][iD]);
				m_vInfo[DataStack.whichDraw][iC][iOff+6] = kldiv;
				m_vInfo[DataStack.whichDraw][iC][iOff+7] = ( (double) vCounts[iC] / iNNVertices ) * kldiv;
			}
		}
	}
	else
	{
		...
		for(iC=1;iC<=iClusts;iC++)
		{
			...
			for(iD=0;iD<iDims;iD++) kldiv += KLDiv(vDistribs[iC][iD],vDistribs[iClusts+1][iD]);			
			//non-weighted inclusive info gain
			m_vInfo[DataStack.whichDraw][iC][0] = kldiv;
			//weighted inclusive info gain
			m_vInfo[DataStack.whichDraw][iC][1] = ( (double) vCounts[iC] / iNNVertices ) * kldiv;
		}
		//uniqueness+distinction distance measure
		vcInf = vector<double>(iClusts+1);
		vector< vector<double> > vcInfInter;
		...
		for(iC=1;iC<=iClusts;iC++)
		{
			m_vInfo[DataStack.whichDraw][iC][2] = vcInf[iC];
			m_vInfo[DataStack.whichDraw][iC][3] = ( (double) vCounts[iC] / iNNVertices ) * vcInf[iC];
		}
		//entropy
		vector<double> vcEntropy(iClusts+1);
		CalcEntropy(vDistribs,vcEntropy,iClusts,iDims,vCounts,iNNVertices,vZeroes,false);
		for(iC=1;iC<=iClusts;iC++)
		{	//un-weighted entropy
			m_vInfo[DataStack.whichDraw][iC][4] = vcEntropy[iC];
			//weighted entropy
			m_vInfo[DataStack.whichDraw][iC][5] = ( (double) vCounts[iC] / iNNVertices ) * vcEntropy[iC];
		}
	}

#ifdef _DEBUG
//	if(false){
//	Distribs2Matlab(vDistribs[0],"_distribs_ml_");
//	Distribs2Matlab(vDistribs[1],"_distribs_ml_");
//	Distribs2Matlab(vDistribs[2],"_distribs_ml_");
//	Distribs2Matlab(vDistribs[3],"_distribs_ml_");
//	}
#endif
}
...
void CalcEXInfo(vector< vector< Hist> >& vDistribs,vector<double>& vcInf,int iClusts,int iDim,vector<int>& vCounts,int iElems)
{
	const bool bUseCounts = false;
	int iC;
	if(!vcInf.size()) vcInf = vector<double>(iClusts+1);
	for(iC=1;iC<=iClusts;iC++)
	{
		double kldiv=0.0;
		int iD=0;
		...
		if(bUseCounts)
			vcInf[iC] = ( (double)vCounts[iC] / (double)iElems ) * kldiv;
		else
			vcInf[iC] = kldiv;
	}
}
		...
		for(iV=0;iV<vClustIDs.size();iV++,iJnk++)
		{
			if(pUp)
			{
				...
				if(iJnk % 25 == 0)
				{
					...
					pUp->SetProgress(msg,99.0 * (double) iV / (double) vClustIDs.size());
				}
			}
			...
			vector<double> vGainSums(iClusts+1);
			...
			if(bUseCounts)
				dGainOut *= (double) (vCounts[iOrig] - 1.0) / (double) iRows;
			...
			for(iT=1;iT<=iClusts;iT++)
			{
				...
				if(bUseCounts)
					dGainIn *= (double) (vCounts[iT] + 1.0) / (double) iRows;
			...
			int iMaxInd = iOrig; double dMaxGain = vGainSums[iOrig];
			for(iT=1;iT<=iClusts;iT++)
			{
				if(iT==iOrig) continue;
			...
			if(iMaxInd != iOrig)
			{
				...
				if(bUseCounts)
					vcInf[iMaxInd] = ((double)vCounts[iMaxInd] / (double)iRows) * dGain;
				else
					vcInf[iMaxInd] = dGain;
				...
				if(bUseCounts)
					vcInf[iOrig] = ((double)vCounts[iOrig] / (double)iRows) * dGain;
				else
					vcInf[iOrig] = dGain;
			}
		}
...
vector<double> CCluster::EntropyInfoClust(CVerxStack& DataStack,int iClusts,int iAlgorithmRuns,const CUPDUPDATA* pCUPDUPData)
{	
	extern int pBins[4];
	const int iBins = pBins[2]; //initialized to 30, but may be modified by user
	int k = 0;
	...
	vector<double> vcEntropyBest(iClusts+1,DBL_MAX / (iClusts+1));
		...
		while(true)
		{
			...
			for(iV=0;iV<vClustIDs.size();iV++,iJnk++)
			{
				...
				for(iT=1;iT<=iClusts;iT++)
				{
					...
					vector<double> vEntropyTemp(vcEntropy);

					vEntropyTemp[iT] =    ((vCounts[iT] + 1.0)/(double)iRows) * SumEntropy(vDistribs[iT],vZeroes);
					vEntropyTemp[iOrig] = ((vCounts[iOrig] - 1.0)/(double)iRows) * SumEntropy(vDistribs[iOrig],vZeroes);
				...
				if(iMinInd != iOrig)
				{
					...
					vCounts[iMinInd]++;
					vcEntropy[iMinInd] = ( (double)vCounts[iMinInd] / (double)iRows) * SumEntropy(vDistribs[iMinInd],vZeroes);

					//move vertex OUT of original distribution
					RemoveVertex(vDistribs[iOrig],pBinData[iV],iDims);
					vCounts[iOrig]--;
					vcEntropy[iOrig] = ( (double)vCounts[iOrig] / (double)iRows) * SumEntropy(vDistribs[iOrig],vZeroes);
...
void CCluster::APClust(CVerxStack& DataStack,int iIter,int iConvIter,double dDamp,double dPref,const CUPDUPDATA* pUp)
{
...
void CCluster::ResistorInfoClust(CVerxStack& DataStack,int iClusts,const CUPDUPDATA* pUp)
{
	...
	vector< vector<double> > vcInfInter(iClusts+1, vector<double>(iClusts+1));
	int iC=1;
	...
	while(true)
	{
		...
		for(iV=0;iV<vClustIDs.size();iV++,iJnk++)
		{
			...
			//now see if moving the vertex to a different cluster had any net positive information gain
			int iMaxInd = iOrig; double dMaxGain = dSumOrig;
			for(iT=1;iT<=iClusts;iT++)
			{
