...
struct decision_function
{
	double *alpha;
	double rho;
	int nSV,	//number of total SVs (SVs + BSVs) 
		nBSV;	//number of BSVs
		
	int *BSV_index, // index of the BSVs in the original dataset
		*SV_index;	// index of the SVs in the original dataset
		
	//nSV - nBSV = the number of SVs only
};
	...
	double Cp, double Cn)
{
	...
	f.nSV = nSV;
	f.nBSV = nBSV;
	f.BSV_index = Malloc(int, nBSV);
	f.SV_index = Malloc(int, nSV - nBSV);
...
// Platt's binary AbstractSVM Probablistic Output: an improvement from Lin et al.
void sigmoid_train(
...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param)
{
	   ...
	   param->svm_type == NU_SVR)
	{
		...
		model->l = f.nSV;
		model->lbounded = f.nBSV;
		
		model->SV_index = f.SV_index;
		model->BSV_index = f.BSV_index;
		
		model->SV = Malloc(svm_node *,f.nSV);
		model->sv_coef[0] = Malloc(double,f.nSV);
		int j = 0;
		int i;
		for(i=0;i<prob->l;i++) {
			if(fabs(f.alpha[i]) > 0) {
...
double kernelfunction(const svm_node *x, const svm_node *y,
				 const svm_parameter& param) {
