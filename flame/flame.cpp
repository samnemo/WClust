// $Id: flame.cpp,v 1.1 2008/11/27 20:04:49 samn Exp $ 
#include "stdafx.h"
/*===================================================================
...
float Flame_SQEuclidean( float *x, float *y, int m )
{
	float d = 0;
	int i;
...
void Flame_LocalApproximation( Flame *self, int steps, float epsilon, const CUPDUPDATA* pUp )
{
	...
	for(t=0; t<steps; t++){
		sprintf(msg,"Clustering Iteration %d of %d",t+1,steps);
		pUp->SetProgress(msg,99.0 * ( t / (double)steps ) );
		dev = 0;
		for(i=0; i<n; i++){
