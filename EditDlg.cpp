// $Id: EditDlg.cpp,v 1.9 2008/11/27 20:06:06 samn Exp $ 
...
BOOL CEditDlg::OnInitDialog() 
{
	...
	AddSzControl( m_BtnCInf, mdRepos, mdRepos);
		...
		for (int i=m_StartSpikes;i<iLim;i++)
		{
			...
			if(i-1<=CountClusts(*m_MainClusters,*m_MainDataStack))
			{
				...
				if(i-1>0 && i-1<m_MainClusters->m_vInfo[m_MainDataStack->whichDraw].size())
				{
					...
					sprintf(myText , "KLD=%.2f ", 
					m_MainClusters->m_vInfo[m_MainDataStack->whichDraw][i-1].m_fBGInfoGain+m_MainClusters->m_vInfo[m_MainDataStack->whichDraw][i-1].m_fInterClustGain);
...
void CEditDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	case IDC_SPIN_X:
		if (m_MyView->m_AxesX != nPos)
		{
			...
			//sprintf(msg,"m_AxesX=%d nPos=%d Dimension=%d BASE_DIMENSION=%d",
			//	m_MyView->m_AxesX,nPos,m_MainDataStack->Dimension,m_MainDataStack->BASE_DIMENSION);
			//MessageBox(msg);
...
{
	vector<string> vEEGNames = m_MainDataStack->GetEEGNames();
	CTimeEv dlg(0,&vEEGNames,m_MainDataStack,m_MainPalette,m_MainClusters);
	dlg.DoModal();
}
...
//	if (m_colDlg->GetSafeHwnd() == 0)
	if ( m_colDlgState == 0 )
	{
		...
		m_colDlg = new CChooseClDlg(0,m_MyView->m_ClustToView);
...
void CEditDlg::OnBnClickedButtonCinf()
{
	// TODO: Add your control notification handler code here
	m_MainClusters->CalcClusterInfo(*m_MainDataStack,true);
}
