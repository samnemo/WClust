// $Id: KK.cpp,v 1.2 2009/06/24 17:57:18 samn Exp $ 
#include "stdafx.h"
//
...
void KK::AlocateCholeskyVecs() {
	...
	kSv.pChol = new vector< FArray > (MaxPossibleClusters, nDims2);
	kSv.pBestChol = new vector< FArray > (MaxPossibleClusters, nDims2);
...
// feature.
bool KK::LoadData(A2D<float>& Dat) {
	int p, i;
	...
	for (i=0; i<nFeatures; i++) UseFeatures[i] = '1';
	...
	for (p=0; p<nPoints; p++) {
		for(i=0; i<nFeatures; i++) {
			Data[p*nDims+i] = Dat[p][i];
		}
	}	
	Dat.Clear(); // free memory here
...
bool KK::LoadData() {
	...
	// calculate number of dimensions
	// permitting use of keyword "all" for UseFeatures
	if (true || strcmp(UseFeatures, "all") == 0) {
		if (nFeatures >= STRLEN)
			Error("Too many features for UseFeatures");
...
float KK::Penalty(int n) {
		...
		//return nParams*log(nPoints)/2;
 		...
 		return (float)(1.0 - penaltyMix) * nParams * 2 + penaltyMix * (nParams * log((double)nPoints)/2);
}
...
void KK::MStep() {
	int p, c, cc, i, j;
	IArray nClassMembers(MaxPossibleClusters);
	FArray Vec2Mean(nDims);
...
void KK::EStep() {
	...
	FArray Vec2Mean(nDims); // stores data point minus class mean
	FArray Root(nDims); // stores result of Chol*Root = Vec
...
void KK::ConsiderDeletion() {
	...
	float Loss, DeltaPen;
	FArray DeletionLoss(MaxPossibleClusters); // the increase in log P by deleting the cluster
