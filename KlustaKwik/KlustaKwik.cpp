// $Id: KlustaKwik.cpp,v 1.3 2009/06/24 17:57:42 samn Exp $ 
#include "stdafx.h"
// KlustaKwik.C
//
...
int MaxClusters = 12;
int MaxPossibleClusters = 20; // splitting can't make it exceed this
int nStarts = 1; // 10; // number of times to start count from each number of clusters
int RandomSeed = 1;
char Debug = 0;
int Verbose = 1;
char UseFeatures[STRLEN] = "11111111111100001";
int DistDump = 0;
float DistThresh = (float)log(1000.0f); // Points with at least this much difference from 
...
// for saving model
int fSaveModel = 0;		// If TRUE, writes cluster info to .model file
...
void SaveOutput(const Array<int> &OutputClass,std::vector<int>& vIDs,int& iMaxClust) {
	int p, c;
#ifdef DONT_COMP
	char fname[STRLEN];
	FILE *fp;
...
try {	// catch out of range subscripts and other goodies
	...
	if(!K1.LoadData(Dat)) return -1; // load .fet file; also sets nDims through call to 
			// AllocateArrays
	...
	if(pUp && pUp->ShouldTerminate()) return -1;
    ...
    if (*StartCluFile) {
		...
		sprintf(msg,"Starting from cluster file %s\n", StartCluFile);
		if(pUp) pUp->SetProgress(msg); Write2Log(msg);

        BestScore = K1.CEM(StartCluFile);
		kSv.BestScoreSave = BestScore;
		
		sprintf(msg,"%d->%d Clusters: Score %f\n\n", K1.nStartingClusters,
				 K1.nClustersAlive, BestScore); Write2Log(msg);
		if(pUp) pUp->SetProgress(msg);

		for(p=0; p<K1.nPoints; p++) {
			K1.BestClass[p] = K1.Class[p];
		}
		SaveOutput(K1.BestClass,vIDs,iMaxClust);

		K1.SaveBestMeans();
	}

	dTotIters = (MaxClusters-MinClusters+1) * nStarts;
	II = 0;
	...
	for(K1.nStartingClusters=MinClusters; K1.nStartingClusters<=MaxClusters; K1.nStartingClusters++) {
	  for(i=0; i<nStarts; i++, II++) {
		// do CEM iteration
        
		sprintf(msg,"Starting from %d clusters: iteration %d of %d...\n", K1.nStartingClusters,i+1,nStarts);
		if(pUp) pUp->SetProgress(msg,99.0*(II/dTotIters)); Write2Log(msg);

		Score = K1.CEM();
		
		sprintf(msg,"%d->%d Clusters: Score %f, best is %f\n", K1.nStartingClusters, K1.nClustersAlive, Score, BestScore);
		if(pUp) pUp->SetProgress(msg); Write2Log(msg);
		
		if (Score < BestScore) {
			
			sprintf(msg,"THE BEST YET!\n");
			if(pUp) pUp->SetProgress(msg); Write2Log(msg);
			...
			if (BestScore < kSv.BestScoreSave) {
				sprintf(msg,"BestScoreSave updated from %f to %f",kSv.BestScoreSave,BestScore);
				if(pUp) pUp->SetProgress(msg); Write2Log(msg);
				kSv.BestScoreSave = BestScore;
			}
	...
	sprintf(msg,"That took %f seconds.", (clock()-Clock0)/(float) CLOCKS_PER_SEC);
	if(pUp) pUp->SetProgress(msg,99); Write2Log(msg);
	...
	if(pUp) pUp->SetProgress("unknown exception thrown");
	Write2Log("kk_main: unknown exception thrown");
}	
	return 0;
}
