...
kmeans(int nclusters, int nrows, int ncolumns, double** data, int** mask,
  ...
  int tclusterid[], int counts[], int mapping[],const CUPDUPDATA* pUp=0)
{ int i, j, k;
  const int nelements = (transpose==0) ? nrows : ncolumns;
  ...
  *error = DBL_MAX;
  ...
  { 
    if(pUp)
	{
		...
		sprintf(msg,"Kmeans with k=%d, pass %d of %d",nclusters,ipass,npass);
		pUp->SetProgress(msg, 99.0 * ((double)ipass / npass));
	}
...
kmedians(int nclusters, int nrows, int ncolumns, double** data, int** mask,
  double weight[], int transpose, int npass, char dist,
  double** cdata, int** cmask, int clusterid[], double* error,
  int tclusterid[], int counts[], int mapping[], double cache[],const CUPDUPDATA* pUp=0)
{ int i, j, k;
  const int nelements = (transpose==0) ? nrows : ncolumns;
  const int ndata = (transpose==0) ? ncolumns : nrows;
  int ifound = 1;
  ...
  *error = DBL_MAX;
  ...
  { 
	...
	{
		if(pUp->ShouldTerminate()) break;
		sprintf(msg,"Kmedians with k=%d, pass %d of %d",nclusters,ipass,npass);
		pUp->SetProgress(msg, 99.0 * ((double)ipass / npass));
	}
...
void CALL kcluster (int nclusters, int nrows, int ncolumns,
  ...
  int clusterid[], double* error, int* ifound, const CUPDUPDATA* pUp)
/*
Purpose
=======
  ...
  *ifound = -1;
  ...
  { double* cache = (double*)malloc(nelements*sizeof(double));
    ...
    { *ifound = kmedians(nclusters, nrows, ncolumns, data, mask, weight,
                         ...
                         tclusterid, counts, mapping, cache, pUp);
      free(cache);
    }
  }
  else
