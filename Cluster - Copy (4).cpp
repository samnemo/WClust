// $Id: Cluster.cpp,v 1.10 2008/02/02 21:20:54 samn Exp $ 
...
bool CheckInfoSettings(bool& bFast,int& iNumN)
{
	FILE* fp = fopen("__wclust__.cfg","r");
	if(!fp)return false;
	bFast=false;
	iNumN=10;
	char buf[1024]={0};
	fgets(buf,1024,fp);
	bFast = (atoi(buf)==0?false:true);
	fgets(buf,1024,fp);
	iNumN = atoi(buf);
	if(iNumN>0)
		return true;
...
bool CIThread(const CUPDUPDATA* pUp)
{
	try
	{
		...
		Clusters.m_vInfo[iWhich] = vector< vector<prob_t> >(iClusts+1);
		for(iC=1;iC<=iClusts;iC++) Clusters.m_vInfo[iWhich][iC] = vector<prob_t>(iInfPerClust);
		...
		bool bFast = false;
		int iNNToFind = 10;
		if(!CheckInfoSettings(bFast,iNNToFind)) 
			bFast=false;
		else Write2Log("CheckInfoSettings bFast=%d iNNToFind=%d",bFast,iNNToFind);

		Write2Log("Starting CalcClustInfo bFast = %d NumElems=%d NumDims=%d",bFast?1:0,iRows,iCols);
		KDTreeHist oTree;
		vector< vector<Neighbor> > vnn;
		if(bFast)
		{
			////////////////////////////////////////////////////////
			...
			//MessageBox(0,"made big tree","wclust",0);
			vnn = vector< vector<Neighbor> >(iRows);
			pUp->SetProgress("Finding initial nearest neighbors...");
			//ScopedTimer S("find 10 nearest neighbors total time");
			int k,j, estsz = 100;
			prob_t startrad = 0.1f, radfct = 2.0f;
			Write2Log("startrad=%.6f radfct=%.6f",startrad,radfct);
			vector<double> dSest(iNNToFind);
			vector<int> vcountest(iNNToFind);
			for(k=0;k<estsz;k++)
			{
				oTree.GetKNN(oTree[k],vnn[k],iNNToFind,startrad,radfct);
			}
			for(j=0;j<iNNToFind;j++)
			{
				for(k=0;k<100;k++)
				{
					if(j<vnn[k].size())
					{
						dSest[j]+=vnn[k][j].m_dist;
						vcountest[j]++;
					}
				}
				if(vcountest[j])
				{
					startrad = dSest[j] / vcountest[j];
					Write2Log("Est. (using %d points) avg dist to neighbor %d = %.6f",estsz,j+1,startrad);
					startrad *= 1.02;
				}
			}
			Write2Log("changed startrad to %.6f radfct=%.6f",startrad,radfct);
			for(;k<vnn.size();k++)
			{
				oTree.GetKNN(oTree[k],vnn[k],iNNToFind,startrad,radfct);
			}
			
			vector<double> dS(iNNToFind);
			vector<int> vcount(iNNToFind);
			for(j=0;j<iNNToFind;j++)
			{
				...
				if(vcount[j]) Write2Log("Avg dist to neighbor %d = %.6f",j+1,dS[j]/vcount[j]);
			}
		}

		//MessageBox(0,"done with knn","wclust",0);
		...
		for(iC=1;iC<=iClusts && !pUp->ShouldTerminate();iC++)
		{
			...
			try
			{
				if(bFast)
				{
					kldiv = FastKLDivSymPNOTP(vDistribs[iC],vCompDistribs[iC],vnn,iC,vClustIDs);
				}
				else
				{
					kldiv = bSymmetric ? 
		...
		Write2Log("Finished CalcClustInfo bFast = %d NumElems=%d NumDims=%d",bFast?1:0,iRows,iCols);
		return true;
	} 
	catch(...)
...
bool CCluster::CalcClusterInfo(CVerxStack& DataStack,bool bSymmetric,HWND wnd)
{
	try
	{
		...
		std::string str("Computing info gain");
...
{	pCUPDUPData->SetProgress("Initializing lookup tables",0);
	ProbInitFree oPIF(DataStack.m_VerxStack.size()+1);
...
prob_t FastKLDivC(KDTreeHist& p,KDTreeHist& notp,vector<vector< Neighbor > >& vNeighbors,int iClustIDP,vector<int>& vClustIDs)
{
	...
	for(iV=0;iV<isz;iV++)
	{
		...
		if(!bFoundP)
		{
			dDistP = p.GetNearestRadius(notp[iT],true);
			iSlow++;
		}
		if(!bFoundNotP)
		{
			dDistNotP = notp.GetNearestRadius(iT);
			iSlow++;
		}
